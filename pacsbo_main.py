...
if __name__ == '__main__':
    ...
    if Furuta:
        ...
        alpha_bar = 1
        hyperparameters = [noise_std, delta_confidence, alpha_bar, m_PAC, gamma_PAC,
                           exploration_threshold, run_type]
        model_path = "rnn_model.pt"
        hidden_size = 20
        num_layers = 2
        num_classes = 1
        global_approach = False
        X_sample_pbo, Y_sample_pbo, global_cube, safety_threshold = run_PACSBO(args=[hyperparameters, num_iterations, X_sample, Y_sample, gt_furuta, X_plot, global_approach, compute_local_X_plot, training])        
        with open('furuta_hardware_X_sample.pickle', 'wb') as handle:
